...
x = requests.post(f'https://dev.sc-dev.io/api/projects/create?organization=tamaragunkel&name={project}&project={project}',
               auth=HTTPBasicAuth(token, ''))
print(x.text)

files = {'report': open(f'/Users/tom/Downloads/{project}.zip','rb')}
x = requests.post(f'https://dev.sc-dev.io/api/ce/submit?projectKey={project}&organization=tamaragunkel',
                    auth=HTTPBasicAuth(token, ''),
                    files=files)
